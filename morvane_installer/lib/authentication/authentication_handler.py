...
from morvane_installer.lib.command import SysCommandWorker
...
class AuthenticationHandler:
	...
	def _configure_u2f_mapping(
		...
		for user in users:
			...
			cmd = ' '.join(
				['artix-chroot', str(install_session.target), 'pamu2fcfg', '-u', user.username, '-o', f'pam://{hostname}', '-i', f'pam://{hostname}']
			)
