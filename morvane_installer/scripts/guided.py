...
from morvane_installer.lib.applications.application_handler import ApplicationHandler
from morvane_installer.lib.args import ArchConfig, ArchConfigHandler
from morvane_installer.lib.authentication.authentication_handler import AuthenticationHandler
...
from morvane_installer.lib.general.general_menu import PostInstallationAction, select_post_installation
from morvane_installer.lib.global_menu import GlobalMenu
from morvane_installer.lib.installer import Installer, accessibility_tools_in_use, run_custom_user_commands
...
from morvane_installer.lib.models.application import Audio, AudioConfiguration
from morvane_installer.lib.models.device import DiskLayoutType, EncryptionType
...
def needs_default_audio(config: ArchConfig) -> bool:
	"""MorvaneOS: whether to set up PipeWire because a desktop was picked with no audio choice.

	app_config is an empty list (not None) when the Applications menu was never
	opened, so test it for truthiness like the rest of this file does.
	"""
	profile = config.profile_config.profile if config.profile_config else None
	if profile is None or not profile.is_desktop_profile():
		return False

	app_config = config.app_config
	return not (app_config and app_config.audio_config is not None)
...
def perform_installation(
	...
	with Installer(
		...
		# This must run before the profile, like the Applications step: desktops pull
		# in a JACK library, and if pacman picks jack2 first, pipewire-jack conflicts.
		if needs_default_audio(config):
